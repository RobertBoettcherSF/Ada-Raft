...
begin
   ...
   Assert (My_Node.Role = Follower, "Initial Role not Follower");
   Put_Line ("  1.2 Assert Initial Term is 0");
   Assert (My_Node.Current_Term = 0, "Initial Term not 0");
   Put_Line ("      PASS");
   ...
   Assert (My_Node.Current_Ticks = 1, "Ticks did not increment");
   Put_Line ("  2.2 Assert Role remains Follower");
   Assert (My_Node.Role = Follower, "Role changed prematurely");
   Put_Line ("      PASS");
   ...
   Assert (My_Node.Role = Candidate, "Role not Candidate after timeout");
   Put_Line ("  3.2 Assert Term incremented");
   Assert (My_Node.Current_Term = 1, "Term not incremented");
   Put_Line ("      PASS");

   -- TEST 4
   Put_Line ("TEST 4 - Candidate Self-Voting");
   Put_Line ("  4.1 Assert Voted_For is self");
   Assert (My_Node.Voted_For = 1, "Candidate didn't vote for self");
   Put_Line ("  4.2 Assert Votes_Received = 1");
   Assert (My_Node.Votes_Received = 1, "Self vote not counted");
   Put_Line ("      PASS");
   ...
   Assert (not RV_Res.Vote_Granted, "Vote granted to stale term");
   Put_Line ("      PASS");
   ...
   Assert (RV_Res.Vote_Granted, "Vote denied to newer valid term");
   Put_Line ("  6.2 Assert node reverted to Follower");
   Assert (My_Node.Role = Follower, "Did not revert to Follower");
   Put_Line ("  6.3 Assert node updated its term");
   Assert (My_Node.Current_Term = 2, "Did not update to newer term");
   Put_Line ("      PASS");
   ...
   Assert (not RV_Res.Vote_Granted, "Voted twice in same term");
   Put_Line ("      PASS");

   -- TEST 8
   Put_Line ("TEST 8 - Reaching Majority becomes Leader");
   Initialize (My_Node, Id => 1, Nodes_Count => 3);
   for I in 1 .. 10 loop Tick (My_Node); end loop; -- Triggers election (Term 1)
   RV_Res := (Term => 1, Vote_Granted => True);
   Receive_Vote_Reply (My_Node, RV_Res);
   Put_Line ("  8.1 Assert 2/3 votes -> Leader Role");
   Assert (My_Node.Role = Leader, "Did not become leader after majority");
   Put_Line ("      PASS");

   -- TEST 9
   Put_Line ("TEST 9 - AppendEntries heartbeats behavior");
   Initialize (My_Node, Id => 1, Nodes_Count => 3); -- Reset to follower, term 0
   My_Node.Current_Term := 1;
   My_Node.Current_Ticks := 5;
   AE_Args := (Term => 1, Leader_Id => 2, Prev_Log_Index => 0, Prev_Log_Term => 0, Entry_Present => False, Entry => (0,0), Leader_Commit => 0);
   Handle_Append_Entries (My_Node, AE_Args, AE_Res);
   Put_Line ("  9.1 Assert Heartbeat resets election ticks");
   Assert (My_Node.Current_Ticks = 0, "Heartbeat failed to reset ticks");
   Put_Line ("  9.2 Assert Success true for valid heartbeat");
   Assert (AE_Res.Success, "Heartbeat rejected");
   Put_Line ("      PASS");
   ...
   Assert (not AE_Res.Success, "Accepted stale AppendEntries");
   Put_Line ("      PASS");

   -- TEST 11
   Put_Line ("TEST 11 - Single Log Entry Append");
   AE_Args := (Term => 3, Leader_Id => 2, Prev_Log_Index => 0, Prev_Log_Term => 0, 
               Entry_Present => True, Entry => (Term => 3, Command => 99), Leader_Commit => 0);
   Handle_Append_Entries (My_Node, AE_Args, AE_Res);
   Put_Line ("  11.1 Assert successful append");
   Assert (AE_Res.Success, "Log append rejected");
   Put_Line ("  11.2 Assert Log Length updated to 1");
   Assert (My_Node.Log_Length = 1, "Log length not updated correctly");
   Put_Line ("  11.3 Assert Log data matches");
   Assert (My_Node.Log(1).Command = 99, "Log command mismatch");
   Put_Line ("      PASS");

   -- TEST 12
   Put_Line ("TEST 12 - PrevLogIndex mismatch Rejection");
   -- Try appending at index 3, but length is 1
   AE_Args := (Term => 3, Leader_Id => 2, Prev_Log_Index => 2, Prev_Log_Term => 3, 
               Entry_Present => True, Entry => (Term => 3, Command => 100), Leader_Commit => 0);
   Handle_Append_Entries (My_Node, AE_Args, AE_Res);
   Put_Line ("  12.1 Assert PrevLogIndex check rejected the entry");
   Assert (not AE_Res.Success, "Accepted entry with missing previous log");
   Put_Line ("      PASS");

   -- TEST 13
   Put_Line ("TEST 13 - Commit Index Advancement");
   -- Append 2nd entry and advance commit
   AE_Args := (Term => 3, Leader_Id => 2, Prev_Log_Index => 1, Prev_Log_Term => 3, 
               Entry_Present => True, Entry => (Term => 3, Command => 100), Leader_Commit => 2);
   Handle_Append_Entries (My_Node, AE_Args, AE_Res);
   Put_Line ("  13.1 Assert successful append at index 2");
   Assert (AE_Res.Success, "Valid sequence append failed");
   Put_Line ("  13.2 Assert commit index matches leader");
   Assert (My_Node.Commit_Index = 2, "Commit Index did not advance");
   Put_Line ("      PASS");
   ...
   My_Node.Role := Leader;
   My_Node.Current_Term := 3;
   AE_Args := (Term => 4, Leader_Id => 2, Prev_Log_Index => 0, Prev_Log_Term => 0, Entry_Present => False, Entry => (0,0), Leader_Commit => 0);
   Handle_Append_Entries (My_Node, AE_Args, AE_Res);
   Put_Line ("  14.1 Assert Leader reverted to Follower");
   Assert (My_Node.Role = Follower, "Leader did not step down on higher term AE");
   Put_Line ("      PASS");
